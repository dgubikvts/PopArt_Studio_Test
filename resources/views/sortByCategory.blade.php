...
<div class="d-flex p-4 justify-content-around">
    ...
    <div class="col-8">
        <h1 class="text-center mb-4">@if(Route::currentRouteName() == 'searchByCategory')Kategorija: {{$category->name}} @else Pretraga: {{$category}} @endif</h1>
        @if(count($ads))
            @foreach($ads as $ad)
        ...
        @else
            <p class="lead text-center">Trenutno ne postoji nijedan oglas u ovoj kategoriji.</p>
        @endif
    </div>
</div>
@endsection